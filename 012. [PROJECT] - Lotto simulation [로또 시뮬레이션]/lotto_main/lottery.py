...
1. 파라미터로 받은 두개의 리스트에서 중복값 개수 반환
 ...
 [generate_numbers(n)]
 랜덤 정수 추첨과정에서 리스트에 이미 있는 값이 나올 경우,
 continue 문을 사용해버려서 파라미터로 받았던 값보다 적은 결과값이 return 되는 상황이 있었다.
 이를 해결하기위해 다중 if문을 적용하고자 하였으나,
 생각을 달리하여 최초사용했던 for 반복문(for i in range(0, n) 대신 while 문을 사용하였다.
 
 [draw_winning_numbers()]
 일반 추첨번호 6개와 보너스 번호 1개를 return 해야한다.
 앞에 작성한 generate_numbers() 함수를 사용하면 간단했지만,
 문제는 보너스 번호 1개가 일반 추첨번호와 중복이 되는지를 어떻게 처리할지였다.
 고민 끝에 코드가 조금 복잡해지긴 했지만, while문과 if문을 이용하여
 중복여부를 검토하고 저장하는 방식으로 결정했다.
 
 * 문제 해결 후 모범답안을 확인해보니 그 코드에서는 그냥 애초에 7개의 랜덤정수를 추출하고,
 그 중에서 [:6]을 일반 추첨번호로 하여 sorting, [6:]을 보너스 추첨번호로 처리했다.
 이렇게 단순하게 생각하니 코드도 내 코드에 비해 훨씬 짧고 간결했다.
 생각하는 연습을 더 해야겠다.
 
 [check(numbers, winning_numbers)]
 상금 조건에 맞추어 if 문으로 판단하는 과정에서 상금번호 리스트의 slicing을 잘못했다.
 winning_numbers[0:6]을 통해 마지막 보너스 번호를 제외한 6개를 count_matching_numbers() 함수로
 보냈어야했는데, [0:5]만 전달하는 바람에 보너스 번호까지 제대로 파악이 되지 않은 것을 모르고 있어서 시간이 오래 걸렸다.
